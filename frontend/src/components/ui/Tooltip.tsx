  ...
  return (
    <span className={cn('relative inline-flex', className)}>
      ...
      {mounted && isVisible && (
        <div
          className={cn(
            'absolute z-50 whitespace-nowrap rounded-md bg-popover px-3 py-1.5 text-sm text-popover-foreground shadow-md border border-border',
  ...
  return (
    <Tooltip {...props}>
      <button
        type="button"
        className={cn(
          'inline-flex items-center justify-center rounded-full p-1 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus:outline-none focus:ring-1 focus:ring-foreground',
          iconClassName
        )}
        aria-label="帮助信息"
      >
